...
class MK_articlesSpider(scrapy.Spider):
    name = 'mk_articles'
    ...
    def start_requests(self):
        ...
        data=[]
        with open('./mk.json') as json_file:
    ...
    def parse(self, response):
        ...
        item['article_title'] = response.xpath('//h1[@class="article__title" and @itemprop="headline"]/text()').extract_first().replace('\n', '')
        temp1=response.xpath('(//div[@class="article__body" and @itemprop="articleBody"])[1]/p/text()').extract()
        temp2=""
        for line in temp1:
